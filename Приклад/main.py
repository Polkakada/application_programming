...
def getInfoNoA(self):
    field_dict = {'Ссылка на видео': self.sourceEdit.text(),
                  ...
                  'Имя создателя': '',
                  'Фамилия создателя': '',
                  'День рождения создателя': '01.01.2200',
                  'Пол создателя': ''}
    return field_dict
...
class InfoForm(QtWidgets.QDialog, infoDialog.Ui_Dialog):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.sourceEdit.setOpenExternalLinks(True)
...
class MainForm(QtWidgets.QMainWindow, mainWindow.Ui_MainWindow):
    ...
    def showItemInfo(self):
        ...
        if re.match(regex, field_dict['Ссылка на видео']) is not None:
            urlLink = "<a href=\"" + field_dict['Ссылка на видео'] + "\">" + field_dict['Ссылка на видео'] + "</a>"
        else:
            urlLink = field_dict['Ссылка на видео']

        self.infoForm.sourceEdit.setText(urlLink)
        self.infoForm.show()
